...
import { heightOf, widthOf } from './utils/family';
...
export function draw(canvas, tree, options = {}) {
        ...
        tree.families.forEach(family => {
            ...
            if (family.type === 'root' || family.type === 'child') {
                family.children.forEach(unit => {
                    ctx.beginPath();
                    ctx.rect((family.X + unit.pos) * X + uP, y + uP + (family.parents.length ? X * SIZE : 0), ((unit.nodes.length * SIZE) * X) - (uP * 2), (X * SIZE) - (uP * 2));
                    ctx.stroke();
                });
            }
        });
    ...
    tree.nodes.forEach(node => {
        ...
        ctx.fillText(node.name, x + X, y + X);
        if (node.hasSubTree) {
            ctx.beginPath();
