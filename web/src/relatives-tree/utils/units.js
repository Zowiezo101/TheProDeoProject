...
export const sameAs = (target) => (unit) => nodeIds(target).join('') === nodeIds(unit).sort().join('');
export const sameAsLeft = (target) => (unit) => nodeIds(target).sort().join('') === nodeIds(unit).join('');
