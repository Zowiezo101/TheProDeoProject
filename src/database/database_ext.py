...
class DatabaseExt:
    ...
    def update_ext(self):
        ...
        if conn is not None:
            ...
            file = open("database/bible.sql", encoding="UTF8")
            sql = file.read()
    ...
    def backup_local(self):
        print("Generating backup for local database in database/bible.sql")
        subprocess.check_output(f'"{self.dba_dump}" --host=localhost --port=3306 --default-character-set=utf8 '
                                ...
                                '> database/bible.sql', shell=True).decode("utf-8")
...
USE `bible`;"""
    ...
    def backup_ext(self):
        print("Generating backup for external database in database/backup.sql")
        subprocess.check_output(f'"{self.dba_dump}" --host={self.dba_host} --port=3306 --default-character-set=utf8 '
                                ...
                                f'"{self.dba_database}" > database/backup.sql', shell=True).decode("utf-8")
