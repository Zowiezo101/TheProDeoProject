...
// TODO: The name field needs be be connected to the search bar
...
function initSlider(field) {
    ...
    var slider = field.slider({
        max: slider_max,
        min: slider_min
    });
    
    // Take the minimum and maximum value by default
    slider_val = [slider_min, slider_max];
    if (field.data("val") !== "") {
        // If a range has previously been selected, restore that selection
        slider_val = field.data("val").split("-");
    }
    
    // Initialize the sliders and set their values
    slider.slider("setValue", slider_val);
...
//        return false;
        ...
        // Get their values and names
        var value = getFieldValue(field);

        // Use the value and name to filter the page results
//        setSearchVal(name, value);
    });
}
